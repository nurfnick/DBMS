...
process_id INT,
assembly_id INT,
CONSTRAINT PK_namufacture PRIMARY KEY (process_id, assembly_id),
...
CONSTRAINT PK_assign PRIMARY KEY(job_num),
CONSTRAINT FK_assign_process FOREIGN KEY(process_id) REFERENCES Processes,
CONSTRAINT FK_assign_job FOREIGN KEY(job_num) REFERENCES Jobs ON DELETE CASCADE,--needed for deleting cut jobs
...
CREATE TABLE Costs(--cost will need a process_id otherwise we won't know where to bill it.
job_num INT,
tran_num INT,
process_id INT,
CONSTRAINT PK_Costs PRIMARY KEY(job_num, tran_num,process_id),
